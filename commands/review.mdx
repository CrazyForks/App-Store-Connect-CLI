...
`--item-type appStoreVersionExperimentV2` is the deprecated singular spelling
of `appStoreVersionExperimentsV2`. It still resolves to the canonical value and
warns on stderr; the alias is removed in 5.0.0.
...
`asc review items list --submission "SUBMISSION_ID"` instead. The removed
`asc review items view` and `asc review items-get` paths stay registered as
stubs that print that migration guidance on stderr and exit with the usage
code; both stubs are deleted in 5.0.0.
