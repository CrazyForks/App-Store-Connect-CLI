...
For compatibility, the legacy monthly `YYYY-MM` and yearly `YYYY` forms remain accepted and are normalized to the period-end date sent to Apple.
...
* `--date` - Report date in `YYYY-MM-DD`; optional for `DAILY`
