...
1. Uploads the build
2. Optionally waits for processing with `--wait`
3. Distributes to specified beta groups

For archive/export/upload/wait without distribution, use the explicit
...
`buildId` with `asc builds add-groups` in a later resumable step.
...
Upload-only JSON is stable for workflow output extraction:
...
If `--wait` or a later optional publish stage fails after the build record is
known, the command still prints partial structured output containing `buildId`,
build metadata, and `error`, then exits nonzero. Resume distribution with the
returned ID instead of uploading the binary again.
