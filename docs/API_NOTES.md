...
- 4.0.0 removes the review item-detail surfaces: `asc review items view` and `asc review items-get` → `asc review items list --submission "SUBMISSION_ID"`; `asc review items update --state` / `items-update --state` → `--resolved` or `--removed`. Both removed commands keep erroring stubs that print this guidance until 5.0.0.
- `appStoreVersionExperimentV2` remains accepted as a deprecated `--item-type` alias for `appStoreVersionExperimentsV2`; it warns on stderr and is scheduled for removal in 5.0.0.
