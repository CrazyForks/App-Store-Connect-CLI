...
func TestReviewsRatingsResolvesBundleID(t *testing.T) {
	originalTransport := http.DefaultTransport
	t.Cleanup(func() {
		http.DefaultTransport = originalTransport
	})
	...
	http.DefaultTransport = roundTripFunc(func(req *http.Request) (*http.Response, error) {
		switch req.URL.Path {
		case "/search":
			if got := req.URL.Query().Get("term"); got != "com.example.alpha" {
				t.Fatalf("expected search term com.example.alpha, got %q", got)
			}
			if got := req.URL.Query().Get("country"); got != "us" {
		...
		case "/lookup":
			if got := req.URL.Query().Get("id"); got != "123" {
				t.Fatalf("expected resolved lookup id=123, got %q", got)
			}
			return &http.Response{
				StatusCode: http.StatusOK,
				Body: io.NopCloser(strings.NewReader(`{
	...
	stdout, stderr := captureOutput(t, func() {
		if err := root.Parse([]string{"reviews", "ratings", "--app", "com.example.alpha", "--output", "json"}); err != nil {
			t.Fatalf("parse error: %v", err)
		}
		if err := root.Run(context.Background()); err != nil {
			t.Fatalf("run error: %v", err)
	...
	if payload.AppID != 123 || payload.AppName != "Alpha" {
		t.Fatalf("unexpected ratings payload: %+v", payload)
	}
}
