...
func TestGameCenterGroupChallengesSetDeprecatedStubRejectsBeforeHTTP(t *testing.T) {
	...
	var runErr error
	stdout, stderr := captureOutput(t, func() {
		if err := root.Parse([]string{"game-center", "groups", "challenges", "set", "--group-id", "group-1", "--ids", "challenge-1,challenge-2"}); err != nil {
			t.Fatalf("parse error: %v", err)
		}
		runErr = root.Run(context.Background())
	})
	...
	if factoryCalled {
		t.Fatal("client factory called for unsupported challenge relationship replacement")
	}
	if stdout != "" {
		t.Fatalf("stdout = %q, want empty", stdout)
	}
	for _, want := range []string{"deprecated", "does not support replacing", "asc game-center challenges create --group-id"} {
...
func TestGameCenterRestoredContractsAppearInHelp(t *testing.T) {
	...
	for _, want := range []string{"DEPRECATED", "--group-id", "--ids", "does not support"} {
